...
_OP_GROUP = {op: group for group, ops in radarr_mcp._GROUPS.items() for op in ops}
...
def test_all_registry_names_grouped():
    ...
    registry_names = [s["name"] for s in radarr_mcp._TOOL_REGISTRY]
    grouped_names = [n for names in radarr_mcp._GROUPS.values() for n in names]
...
async def test_group_tools_are_the_only_registered_tools(server):
    ...
    assert {t.name for t in tools} == set(radarr_mcp._GROUPS)
...
async def test_unknown_operation_rejected_by_schema(server):
    ...
    with pytest.raises(ToolError, match="validation error"):
        async with Client(server) as c:
            await c.call_tool("radarr_tags", {"operation": "not_a_real_operation"})
